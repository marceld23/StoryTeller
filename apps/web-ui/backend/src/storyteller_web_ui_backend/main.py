...
def create_session(payload: CreateSession) -> CreatedSession:
    ...
    # Don't trigger engine.opening() here. The WS handler does it on
    # first connect and emits a "thinking" signal while it runs. If
    # we also generated the opening here, the client would render it
    # twice: once from the REST response (intro + first scene combined)
    # and once from the WS auto-send of last_narration on connect
    # (which has only the first scene, because the intro generated by
    # world_intro() is intentionally NOT persisted to memory). The
    # dedup check on the client compares full strings and can't see
    # that the second one is a suffix of the first.
