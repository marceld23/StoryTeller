...
class OpenAITTS(TTS):
    ...
    def synthesize(self, text: str, instructions: str = "") -> tuple[np.ndarray, int]:
        ...
        base = self.cfg.models.tts_endpoint.base_url or ""
        log.info("TTS: model=%s voice=%s endpoint=%s", self.cfg.models.tts,
                 self.cfg.models.tts_voice, base or "OpenAI")
        ...
        try:
            ...
            err = wrap("tts", base_url=base,
                       model=self.cfg.models.tts)(exc)
            HealthRegistry.get(self.cfg).record_error(err)
            raise err from exc
        HealthRegistry.get(self.cfg).record_ok(
            "tts", base_url=base, model=self.cfg.models.tts)
        import io
