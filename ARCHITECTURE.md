...
- **Boot flow** (in [apps/pi/.../main.py](apps/pi/src/storyteller_pi/main.py), `cmd_run`) — plays the cached `intro` greeting + (separately toggled) `intro_commands` listing the in-story voice commands; both flags live in `data/settings.json`. Then the device **idles silently** until *"Hey Jarvis"* fires. On wake-word it asks `start_question` (*"Möchtest du eine Geschichte starten?"*) and only on a *yes* hands off to the voice menu. *No* / unclear (after one re-ask) drops back to the wake-word wait.
...
- **Mode branch & voice-mode world design** — after `_await_start_yes` the loop asks `mode_question` ("existing or new?") via `classify_play_mode()`. *Create* hands off to `_design_world_interactively()`: a [`WorldDesignInterview`](packages/core/src/storyteller_core/story/world_design.py) drives the Q&A (one `gen`-LLM call per turn, cap-checked); the player ends the interview with the `generate` voice keyword; the assembled brief feeds the existing [`generate_world`](packages/core/src/storyteller_core/worlds/generate.py) pipeline under a neutral `generic_waiting.wav` ambient, the freshly saved world is RAG-indexed immediately, and the player confirms `start / back to menu`. The interview is also persisted as `data/transcripts/_world_design-<utc-ts>.jsonl` for audit.
