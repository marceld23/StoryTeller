...
- **Boot flow** (in [apps/pi/.../main.py](apps/pi/src/storyteller_pi/main.py), `cmd_run`) — plays the cached `intro` greeting (toggleable via `data/settings.json`), then **idles silently** until *"Hey Jarvis"* fires. On wake-word it asks `start_question` (*"Möchtest du eine Geschichte starten?"*) and only on a *yes* hands off to the voice menu. *No* / unclear (after one re-ask) drops back to the wake-word wait — the device stays unobtrusive when nobody wants to play.
